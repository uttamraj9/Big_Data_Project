...
resource "local_file" "gold_views_sql" {
  ...
  content  = <<-SQL
    ...
    WITH (LOCATION = 'abfss://gold@${var.adls_account_name}.dfs.core.windows.net');
    GO
    ...
    FROM OPENROWSET(BULK 'cc_fraud_trans/**', DATA_SOURCE='gold_adls', FORMAT='PARQUET')
    WITH (merchant_category VARCHAR(100), transaction_amount FLOAT, fraud_label INT) AS t
    ...
    SELECT device_type,
           ...
           ROUND(AVG(CAST(fraud_label AS FLOAT))*100, 2)   AS fraud_rate_pct
    FROM OPENROWSET(BULK 'cc_fraud_trans/**', DATA_SOURCE='gold_adls', FORMAT='PARQUET')
    WITH (device_type VARCHAR(50), fraud_label INT) AS t
    ...
    SELECT CAST(txn_date AS DATE)                           AS txn_date,
           ...
           ROUND(AVG(CAST(fraud_label AS FLOAT))*100, 2)   AS fraud_rate_pct
    FROM OPENROWSET(BULK 'cc_fraud_trans/**', DATA_SOURCE='gold_adls', FORMAT='PARQUET')
    WITH (txn_date VARCHAR(20), transaction_amount FLOAT, fraud_label INT) AS t
    ...
    FROM OPENROWSET(BULK 'cc_fraud_trans/**', DATA_SOURCE='gold_adls', FORMAT='PARQUET')
    WITH (location VARCHAR(100), transaction_amount FLOAT, risk_score FLOAT, high_risk INT) AS t
    WHERE high_risk = 1
    GROUP BY location;
    GO
  SQL
